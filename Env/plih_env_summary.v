...
FOR STUDENTS:
#<ol>#
#<li>#Finish the IDs chapter first - this chapter reuses its BAE language, [subst], and [eval].#</li>#
#<li>#Read plih_env_lecture.v.#</li>#
#<li>#Work plih_env_exercises.v ([Admitted] -> [Qed]).#</li>#
#<li>#Check against plih_env_solutions.v.#</li>#
#</ol>#

FOR INSTRUCTORS:
#<ol>#
#<li>#Read plih_env_instructor_guide.v.#</li>#
...
(**
An _environment_ is a table of identifier/value bindings.  Instead of
substituting a value the moment a [Bind] is seen, we remember the
binding and look it up when we reach an [Id].  This is a pure
_efficiency_ optimization: it does not change _what_ eval computes.
...
([\t -> eval [] t == evals t]); we prove it for _all_ terms.
...
LAYER 2: Lecture (plih_env_lecture.v)
  ...
  Section 8: Concrete syntax (_inherited_) - the [<{ ... }>] parser from
...
LAYER 3: Exercises (plih_env_exercises.v)
  ...
  _prelude_ interpreter and an error-reporting interpreter [evalErr] (with
...
next adds _functions_ (first-class [lambda]/application), where the
