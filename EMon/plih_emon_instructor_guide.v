...
_failure_ half of the monad, so frame it as "same threading, richer
errors".
 *)
...
(**
#<ol>#
#<li>#_The complaint._ RMon rejects [Plus (Boolean true) (Num 1)] with [None].  A real compiler says _where_ and _why_.  We want messages.#</li>#
#<li>#_Stack Either on Reader._ [RE E A = E -> string + A].  Walk the operations, contrasting with RMon: [retE]/[askE]/[localE] mirror the Reader ones; the difference is [bindE] threads [inl msg] through (short-circuit) and [throwE msg] replaces [failR].  Success is [inr], failure is [inl] with a message.#</li>#
#<li>#_The checker._ [typeofE] is [typeofR] with each [failR] turned into a descriptive [throwE].  Show a couple of error strings; note the logic is byte-for-byte the same otherwise.#</li>#
#<li>#_Refinement._ Introduce [forget : string + A -> option A].  The theorem [typeofE_refines : forget (typeofE e ctx) = typeof ctx e] says the messages are pure _added value_ - erase them and you are back to the exact [option] checker.  So no program's accept/reject status changed; only the diagnostics improved.#</li>#
#<li>#_The proof shape._ Same induction as RMon's agreement, but because [forget] sits on the _outside_ we rewrite the direct side _backwards_ ([rewrite <- IH]) to expose [forget (typeofE ...)] and then case on the [sum] ([inl]/[inr]).  Worth showing once; the rest is mechanical.#</li>#
#</ol>#
 *)
...
(**
  ...
  - Messages are _data_, not control.  The refinement theorem is exactly the
...
Part 4 (ex9-ex11): concrete syntax (Section 6).  TRec's two notations -
  ...
  _message-carrying_ [typecheckE].  ex10 succeeds on [inr]; ex11 shows a
...
when the course adds mutable _state_ - there the threaded thing is a store.
 *)
