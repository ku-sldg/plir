...
(**
#<ol>#
#<li>#_The missing piece._ Recall from Func: [omega] loops, but FBAE has no conditional, so a recursive function can never test its argument and stop.  The fix is a _conditional_.  Add [Boolean], [IsZero], [If] (and [Mult] for factorial), giving FBAEC.  Stress that [If] runs _only_ the selected branch - that is what lets recursion bottom out ([ev_if_lazy_branch] shows the untaken branch is never evaluated).#</li>#
#<li>#_Two interpreters, one story._ Reintroduce the strict [evalM] and lazy [evalL] from Func, now with the conditional cases.  Nothing conceptually new here - it is deliberately parallel to Func so the recursion payoff stands out.  Re-prove fuel monotonicity so students see the extra cases ([Mult]/[Boolean]/[IsZero]/[If]) are routine.#</li>#
#<li>#_omega -> recursion._ The key conceptual move: [omega] is useless self-application; a _fixpoint combinator_ is the _same_ trick parameterised by the function to iterate.  Introduce Y, then observe it is a parameterised [omega]: under strict evaluation it diverges.#</li>#
#<li>#_The eta-guard._ Show _why_ strict Y loops (call-by-value forces the argument [x x] before doing any work) and how Z fixes it by hiding the self-application behind [\v. x x v], which is a value.  Z is the call-by-value fixpoint.#</li>#
#<li>#_Payoff._ Run summation and factorial to real answers - Z under strict [evalM], Y under lazy [evalL].  This is the moment the course delivers on "functions are enough for recursion."#</li>#
#</ol>#
 *)
...
(**
  ...
  - _Literal fuel on an abstract term._ In proofs, [evalM <literal> env e]
    with [e] a variable makes the kernel try to unroll [evalM] to that
    depth and can blow up.  Keep fuel a _variable_ in any lemma quantified
    over an abstract term (see [ex9_more_fuel]).  A literal fuel is fine
    only on a _concrete_ closed term, where evaluation computes to a value
    and stops (that is why all the [reflexivity] examples work).

  - _Fuel too small._ The recursive examples need enough fuel: [eval] uses
    ...
    at _any_ fuel - use a modest amount (100) to keep proofs fast.
...
Part 4 (ex11-ex14) - concrete syntax (Section 8).  FBAEC gets its _own_
  ...
  Common mistake: application binds _tighter_ than [*]/[+], so [g z - 1]
