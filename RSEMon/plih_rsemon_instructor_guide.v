...
chapter contributes NO new individual effect - the whole lesson is that
three known effects COMPOSE into one monad.  A quick "which effect does
...
(**
1. THE MISSING CHANNEL.  RSMon hid the environment and store but still
   failed SILENTLY.  Recall EMon: an Either layer turns [None] into a
   descriptive message, and forgetting the message recovers the plain
   answer.  Goal: add that third channel to RSMon.

2. THE THREE-EFFECT MONAD.  Define
   [RSE E S A := E -> S -> sum string (A * S)].  Read the shape aloud:
   given an environment and a store, EITHER a message ([inl]) OR a value
   with the updated store ([inr]).  Give the three op groups - Reader
   ([askRSE]/[localRSE]), State ([getRSE]/[putRSE]), Either ([throwRSE]) -
   and the one [bindRSE] that threads env and store through [inr] and
   short-circuits on [inl].  Define [forget] exactly as in EMon.

3. THE MONADIC INTERPRETER.  Rewrite the interpreter as [evalRSE].  It is
   RSMon's [evalRS] with every silent failure replaced by a descriptive
   [throwRSE] - unbound identifier, non-number operand, non-Boolean
   condition, non-function application, non-location deref/assign, and
   "out of fuel" at the base case.  Nothing else changes.

4. REFINEMENT.  Prove [forget (evalRSE fuel e env s) = evalM fuel env s e]
   by induction on fuel.  Emphasise the shape of the statement: the extra
   messages are provably INVISIBLE once forgotten.  This is the same
   refinement idea as EMon, now for the (fuel-driven, stateful) evaluator.

5. LAWS.  Close with left identity, Either short-circuit
   ([throw_short_circuits]), and the three-channel independence lemma
   ([channels_independent]).  The takeaway: each effect keeps its own
   behavior while sharing one [bind] - the essence of monad transformers.
 *)

(** * PART 3: COMMON PITFALLS *)

(**
- WHY [sum string (A * S)] AND NOT [(sum string A) * S]?  On failure the
  store is DISCARDED (the whole computation is [inl msg]); there is no
...
- "OUT OF FUEL" IS AN ERROR TOO.  At fuel 0 the interpreter [throwRSE]s
  "out of fuel".  Under [forget] this is [None] - exactly [evalM 0].  So
  the Either channel carries BOTH genuine type errors and resource
  exhaustion; [forget] erases the distinction, which is why refinement
  holds.

- THE REFINEMENT PROOF.  Same recipe as EMon, threaded through fuel and
  ...
  key step - [cbn [forget]] AFTER the destruct so the [forget (inr ...)]
...
- LAWS BY [reflexivity].  Left identity, short-circuit, and the
...
(**
The moral, complete: effects COMPOSE.  Reader, State, and Either each
